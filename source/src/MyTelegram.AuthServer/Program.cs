...
Log.Information("{Info} {Version}", "MyTelegram Auth Server", typeof(Program).Assembly.GetName().Version);
...
builder.ConfigureServices((context,
    services) =>
{
    services.Configure<MyTelegramAuthServerOptions>(context.Configuration.GetRequiredSection("App"));
